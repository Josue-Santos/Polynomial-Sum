...
Compare Polynomial::compare(Polynomial p)
{
	...
	unsigned int exp = max(a.degree(), p.degree()); //10
	...
	while (exp > 0 && a[exp] == p[exp]) {
		...
		--exp; //5 10x^10 = exp 5 t2= 0 //5 p1 = -1000
	}
	//std::cout << "a = " << a[exp] << " p = " << p[exp] << std::endl; //found the coefficient of a and p

	//checking for any negative coefficient with the same degree when exp is the least value possible
	...
	if (a[exp] < 0 && p[exp] < 0) { // when the coeffient of a (this) is negativity 
		//getting the values for less, greater and equal
		if (a[exp] > p[exp]) {
			//less
			return Compare::greater;
...
bool Polynomial::operator == (Polynomial p) {
	return compare(p) == Compare::equal;
}
...
//Less Than Operator
bool Polynomial::operator < (Polynomial p) {
	return compare(p) == Compare::less;
}

//Greater Than Operator
bool Polynomial:: operator > (Polynomial p) {
	return compare(p) == Compare::greater;
}
...
bool Polynomial::operator == (Term t) {
	Polynomial q(t);
	return compare(q) == Compare::equal;
}
...
bool Polynomial::operator < (Term t) {
	Polynomial q(t);
	return compare(q) == Compare::less;
}

//Greater Than Operator
bool Polynomial:: operator > (Term t) {
	Polynomial q(t);
	return compare(q) == Compare::greater;
}
...
double Polynomial::operator() (int x) {
	...
	while (exp > 0) {
		...
		--exp;
		//reaches until the exp 1 
	}
	total += copy[exp]; //needed to calculate the polynomial when degree is 0
	return total;
}
